...
exports.addReview = asyncHandler(async (req, res, next) => {
  ...
  if (!bootcamp) {
    return next(
      new ErrorResponse(
        `Bootcamp não encontrado: ${req.params.bootcampId}`,
        404
      )
    );
  }
...
exports.updateReview = asyncHandler(async (req, res, next) => {
  ...
  if (!review) {
    return next(
      new ErrorResponse(`Review não encontrado: ${req.params.id}`, 404)
    );
  }
  ...
  if (review.user.toString() !== req.user.id && req.user.role !== "admin") {
    return next(
      new ErrorResponse(`Sem autorização para atualizar review`, 404)
    );
  }
